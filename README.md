...
- `COM3D2.MotionTimelineEditor_SceneCapture.Plugin.dll`
  - 配置するとSceneCaptureプラグインで動作するようになります。任意で配置してください(SceneCaptureプラグインが必要)
- `COM3D2.MotionTimelineEditor_PartsEdit.Plugin.dll`
  - 配置するとPartsEditプラグインで動作するようになります。任意で配置してください(PartsEditプラグインが必要)
...
- サブウィンドウの挙動修正
  ...
  - レイヤー切替時に自動で情報ウィンドウに切り替えるように
- モデル設定をモデル情報ウィンドウ内のタブに移動
  - モデルの表示/非表示制御も追加
- 数値入力の挙動修正
...
- マウススクロール時の挙動修正
  ...
  - スクロールビューの非アクティブ時にもマウススクロールできるように
- 背景の軸毎の拡縮対応
- 複数メイドの編集モード切り替えでIK表示も連動させる
- コンボボックスのレイアウト調整
- メイド表情をタブ化
