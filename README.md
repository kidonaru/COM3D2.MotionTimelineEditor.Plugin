...
- **カメラの補間にTangentを使用**: カメラの補間にTangentを使用します
- **ライトで色補間を有効化**: ライトの色補間を有効化します
- **オフセット時間**: DCM出力時の開始/終了時間のオフセットを設定
- **フェード時間**: DCM出力時のフェードイン/アウト時間を設定
- **アスペクト比**: アスペクト比を設定、設定するとレターボックスが表示されます
