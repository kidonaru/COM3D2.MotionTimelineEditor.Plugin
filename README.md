...
- [COM3D2.MotionTimelineEditor.Plugin](#com3d2motiontimelineeditorplugin)
  ...
  - [サブウィンドウ](#サブウィンドウ)
    ...
    - [テンプレウィンドウ](#テンプレウィンドウ)
      - [テンプレ操作](#テンプレ操作)
      - [テンプレカテゴリ編集](#テンプレカテゴリ編集)
      - [テンプレ編集](#テンプレ編集)
      - [テンプレの保存フォーマット](#テンプレの保存フォーマット)
    - [操作履歴ウィンドウ](#操作履歴ウィンドウ)
    - [設定ウィンドウ](#設定ウィンドウ)
      - [個別設定](#個別設定)
      - [1フレーム調整](#1フレーム調整)
  ...
  - [保存形式](#保存形式)
    ...
    - [連番画像出力](#連番画像出力)
  - [変更履歴](#変更履歴)
    - [2026/08/03 v5.1.0.1](#20260803-v5101)
    - [2026/08/02 v5.1.0.0](#20260802-v5100)
    - [2025/06/07 v5.0.0.1](#20250607-v5001)
    - [2025/06/04 v5.0.0.0](#20250604-v5000)
    - [2025/04/06 v4.9.0.0](#20250406-v4900)
    - [2025/03/23 v4.8.0.1](#20250323-v4801)
...
ModItemExplorerプラグインで、モデルの配置や、複数アニメレイヤーでのモーション再生が可能です。
(アニメレイヤー機能はv1.6.0.0で実装されています)

- **ModItemExplorer 1.6.0**
  - https://github.com/kidonaru/COM3D2.ModItemExplorer.Plugin/releases
...
- [サブカメラレイヤー](#サブカメラレイヤー): サブカメラ(ワイプ表示)の制御
...
https://github.com/user-attachments/assets/03f95a5c-3a82-4fee-a7b1-4bee1ff55122

別アニメーションをブレンドして再生することができます。
各アニメレイヤー毎にファイルを読み込んで再生します。

ModItemExplorer v1.6.0.0以降で複数アニメレイヤーでの再生機能が追加されているので、基本的にModItemExplorerでの編集を想定しています。

- **ループ**: ループ再生の有効/無効を切り替え
- **時間上書き**: 基本的に開始時間は再生時にしか参照しないですが、
  ここにチェックを入れると、開始時間を上書きします。
- **開始時間**: アニメーションの開始時間を設定
- **重み**: アニメーションの重みを設定
- **再生速度**: アニメーションの再生速度を設定
...
メインカメラとは別のサブカメラを画面上にワイプ(小窓)として表示し、位置/角度/視野角/表示範囲をタイムラインで制御することができます。

- サブカメラはレイヤー追加時に1台自動で作成され、最大8台まで追加できます
- レイヤーを削除すると、作成したサブカメラも全て削除されます

各種操作:
- **カメラ選択**: 操作対象のサブカメラを選択
- **追加**: サブカメラを追加 (最大8台)
- **削除**: 末尾のサブカメラを削除 (最低1台は残ります)
- **有効**: サブカメラの表示/非表示を切り替え
- **追従メイド**: 指定したメイドにサブカメラを追従させます
  - 「なし」を選択すると追従を解除します
- **位置 (X/Y/Z)**: サブカメラの位置を変更
  - 追従メイド設定時は、メイドからのオフセットとして扱われます
- **RX/RY/RZ**: サブカメラの角度を変更
- **FoV**: サブカメラの視野角を変更

ビューポート設定:
- **X/Y**: 画面上のワイプ表示位置を設定 (0~1)
- **幅/高さ**: ワイプの表示サイズを設定 (0~1)
...
![GTToneMap](img/img_60.jpg)

GTトーンマップを適用することができます。
...
![テンプレ操作](img/img_61.jpg)

テンプレの適用、追加をすることができます。
テンプレはレイヤーごとに登録されます。

- **テンプレ名**: 登録するテンプレの名前を設定
- **追加**: 選択したキーフレームをテンプレとして登録
- **テンプレボタン**: 登録したテンプレを適用します
  - 適用すると、現在のフレームにテンプレの値が設定されます


#### テンプレカテゴリ編集

![テンプレカテゴリ編集](img/img_62.jpg)

カテゴリの編集ができます。

- **カテゴリ名**: 追加するカテゴリの名前を設定
- **追加**: カテゴリを追加
- **削除**: カテゴリを削除
- **↑/↓**: カテゴリを上下に移動
...
登録したテンプレは下記パスに保存されます。
```
Sybaris\UnityInjector\Config\MotionTimelineEditor\Template
```

レイヤーごとにXMLファイルが保存されます。
```
<TemplateLayer xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <LayerName>MorphTimelineLayer</LayerName>
  <Category>
    <CategoryName>口</CategoryName>
    <Template>
      <TemplateName>う1</TemplateName>
      <Frame>
        <FrameNo>1404</FrameNo>
        <Bone>
          <Transform>
            <Name>moutha</Name>
            <Type>Morph</Type>
            <Value>0.43589744</Value>
            <InSmoothBit>0</InSmoothBit>
            <OutSmoothBit>0</OutSmoothBit>
          </Transform>
        </Bone>
```
...
- サブカメラレイヤーの追従カメラに追従ポイント選択と向き反映を追加
- 背景モデルレイヤーで補完曲線が適用されないバグの修正


### 2026/08/02 v5.1.0.0

- サブカメラレイヤーの追加
- 拡張ボーンのスケール値をanm出力から除外
- モデル強制更新時にタイムラインモデルの同期漏れがあるバグの修正
- 中心点IKの表示条件の修正
- モデル追加時にNREが発生する場合があるバグの修正 (スタジオモード)
- NPRShader使用時にNREが発生する場合があるバグの修正


### 2025/06/07 v5.0.0.1

- 簡易設定の中心点IK表示/関節IK表示を追加 (スタジオモードのみ)
- IK表示切り替え時に一部IKが残るバグの修正 (MeidoPhotoStudioのみ)


### 2025/06/04 v5.0.0.0

- テンプレート機能の追加
- ポストエフェクトにGT Tonemap追加
- モデルボーンでボーン名に_nubがあるボーンを編集対象外に修正
- 簡易設定にモデル表示切り替えの追加
- 簡易設定の中心点IK表示/関節IK表示を削除、スタジオモードでも自動でIK表示制御するように
- 編集時のIK表示はメイドアニメ/移動レイヤーでのみ表示するように
- モデルのマテリアル変更が反映されない場合があるバグの修正
- 一つ戻すをしたときに情報レイヤーのタブが初期化されるバグの修正
...
- メイドアニメブレンドレイヤーの追加
  - 別アニメーションをブレンドして再生することができます
  - 各アニメレイヤー毎にファイルを読み込んで再生します
  - ModItemExplorer v1.6.0.0以降で複数アニメレイヤーでのアニメ再生制御ができます
...
- メイドマテリアルレイヤー追加
  ...
  - メイド衣装レイヤーで固定化した衣装に対して変更することを推奨
