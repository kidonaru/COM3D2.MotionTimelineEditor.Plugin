...
- **X, Y, Z**: 選択したキーフレームの位置を調整
  ...
  - `<<` `<` `>` `>>`で、0.1、0.01ずつ相対的に移動します
- **RX, RY, RZ**: 選択したキーフレームの角度を調整
  - `<<` `<` `>` `>>`で、10、1ずつ相対的に変更します
- **初期化**: 選択したキーフレームの位置と角度を初期化
- **補正曲線**: 選択したキーフレームの補正曲線を表示します
- **表示種類**: 右の選択ボタンで、表示する補正曲線の種類を切り替えます
...
- **Tangent範囲**: 補正曲線の範囲を設定
- **初期化**: タイムライン個別設定を初期化

- **共通設定**: タイムライン共通の設定を行います
- **初期補正曲線**: 新規で追加するキーフレームの補正曲線を設定
- **Trans詳細表示数**: キーフレーム詳細ウィンドウで表示するTransformの最大数
  - キーフレーム詳細が重い場合は下げると改善することがあります
- **Tangent表示数**: キーフレーム詳細ウィンドウで表示する補正曲線の最大数
...
2024/04/03 v1.1.0.0
- キーフレームの一括値変更機能を追加
  - メインメニューの「キーフレーム」から変更できます
- 補正曲線の設定機能を追加
  - デフォルトが自動補正ありになっているため、前のバージョンと動きが変わります
