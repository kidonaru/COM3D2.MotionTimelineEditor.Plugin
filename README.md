...
- **IKのアニメ制御**: アニメ再生中もIKが有効になります
- **足の接地処理**: 足首の位置を地面に固定します (足首IKを固定する必要があります)
- **床の高さ**: 地面の高さを設定
...
1フレームの長さのキーフレームを配置した場合、通常は1フレーム間で補完が走りますが、0フレームの長さとして扱うことで即時に切り替えることができます。

調整方法として前フレームを伸ばすのと、後フレームを伸ばす方法の2種類があります。

- 調整なし
- 前フレームを拡張 (デフォルト)
- 後フレームを拡張

v2.8.0.0以前に作成されたタイムラインは`後フレームを拡張`、v2.9.0.0以降に作成されたタイムラインは`前フレームを拡張`がデフォルト挙動です。
